...
contract Token is Ownable, ERC20, AccessControl {
  ...
  function addRetriever(address retriever) external onlyOwner {
    require(!hasRole(retrieverRole, retriever), "already_retriever");
    _grantRole(retrieverRole, retriever);
  }

  function removeRetriever(address retriever) external onlyOwner {
    require(hasRole(retrieverRole, retriever), "not_retriever");
    _revokeRole(retrieverRole, retriever);
  }
